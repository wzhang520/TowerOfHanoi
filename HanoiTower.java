...
public class HanoiTower {
	...
	public void solvePuzzle(String[]args) {
		...
		cursedTower(levels, 0, 1, 2);
	}

   /**
    *  Recursive algorithm that solves the hanoi tower
    *  @param stack      the level of tower to solve
    *  @param startPeg      the peg that the algorithm starts from
    *  @param endPeg        the peg that the algorithm ends on
    *  @param offPeg        the other peg that the algorithm uses
    */
	public void cursedTower(int stack, int startPeg, int endPeg, int offPeg){
		if(stack == 1){ // special case
				moveDisk(startPeg, endPeg);
		} else if(stack == 2){ // base case
			moveDisk(startPeg, offPeg);
			printTowers();
			moveDisk(startPeg, endPeg);
			printTowers();
			moveDisk(offPeg, endPeg);
			printTowers();
		} else {
			cursedTower(stack-1, startPeg, offPeg, endPeg); // move stack to off peg
			moveDisk(startPeg, endPeg); printTowers(); // move bottom piece to middle
			cursedTower(stack-1, offPeg, endPeg, startPeg); // move stack from off peg to middle
		}
	}
